    ...
    clientFn  = { c => c.copy(clients = Seq(TLClientParameters(
      ...
      userBits    = {
        require( c.clients.forall( _.userBits.length == c.clients(0).userBits.length ),
          s"TLFragmenter (with parent $parent): Length of userBits sequences of all clients must be equal. ${c.clients.map(x => (x.name, x.userBits.length))}")
        require( c.clients.forall( _.userBits.zip( c.clients(0).userBits ).forall { case (a, b) => a.width == b.width } ),
          s"TLFragmenter (with parent $parent): Width of corresponding userBits for all clients must match. ${c.clients.map(x => (x.name, x.userBits))}")
